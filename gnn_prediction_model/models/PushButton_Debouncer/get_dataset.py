...
class GNNDataset_criticalpath(InMemoryDataset): 
    ...
    def raw_file_names(self):
        return os.listdir('/home/wllpro/llwang/yfdai/HRAE_company/raw_crit_paths/PushButton_Debouncer/')  # 遍历 JSON 文件

    @property
    def processed_file_names(self):
        return ['PushButton_Debouncer.pt']

    def download(self):
        pass

    def process(self):
        report_df = pd.read_csv('/home/wllpro/llwang/yfdai/HRAE_company/raw_dataset_results/PushButton_Debouncer.csv')
        data_list = []

        for json_file in tqdm(self.raw_file_names, desc='Processing JSON Files'):
            if json_file.endswith('.json'):
                json_file_path = f'/home/wllpro/llwang/yfdai/HRAE_company/raw_crit_paths/PushButton_Debouncer/{json_file}'
                arch_name = json_file.replace('.json', '')
                # 查找对应的 XML 文件
                if f'{arch_name}.xml' in os.listdir('/home/wllpro/llwang/yfdai/HRAE_company/raw_dataset_archs/'):
                    ...
                    if not label_row.empty:
                        ...
                        xml_file_path = f'/home/wllpro/llwang/yfdai/HRAE_company/raw_dataset_archs/{arch_name}.xml'
