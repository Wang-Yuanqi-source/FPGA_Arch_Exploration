...
class GNNDataset_criticalpath(InMemoryDataset): 
    ...
    def raw_file_names(self):
        return os.listdir('/home/wllpro/llwang/yfdai/HRAE_company/raw_crit_paths/GrayCounter/')  # 遍历 JSON 文件

    @property
    def processed_file_names(self):
        return ['GrayCounter.pt']

    def download(self):
        pass

    def process(self):
        report_df = pd.read_csv('/home/wllpro/llwang/yfdai/HRAE_company/raw_dataset_results/GrayCounter.csv')
        data_list = []

        for json_file in tqdm(self.raw_file_names, desc='Processing JSON Files'):
            if json_file.endswith('.json'):
                json_file_path = f'/home/wllpro/llwang/yfdai/HRAE_company/raw_crit_paths/GrayCounter/{json_file}'
